...
let test_of_list =
  ...
  let actual = O.to_list (O.get_tree root) |> sort_ggv3_list in
...
let test_nearest_handpicked_failures =
  let make points target =
    ...
    let result = O.nearest (O.get_tree root) target in
    let* _ =
...
let test_nearest_sample_empty =
  ...
  expect_raises (fun () -> O.nearest (O.get_tree root) p) exc_f Gg.V3.pp
