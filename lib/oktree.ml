...
*)
module Make : MAKER = functor (V3 : VEC3) ->
struct
  type vec3 = V3.t
  ...
  *)
  ...
  let insert okt pt =
    let path = path_to pt okt.tree in
    let insert' = function
      | Leaf l -> from_list' okt.leaf_size (pt :: l)
      | Node _ ->
        raise @@ Invalid_argument "Cannot insert into a Node - inserts can only be performed on Leaf nodes"
    in
    { okt with tree = apply_by_path insert' path okt.tree }
  ...
  *)
  ...
  let _distances okt p =
    List.map (fun p' -> (p', distance p p')) (to_list okt)

  let rec _print_centres ?(label = "Root") tree =
    ignore
    @@
    match tree with
    | Node n ->
      ...
      List.iter
        (fun o ->
           ...
           _print_centres ~label:(show_octant o) child)
        all_octants
    | Leaf _ -> ()

  let rec _print_centre_distances ?(label = "Root") tree p =
    ignore
    @@
    match tree with
    | Node n ->
      ...
      List.iter
        (fun o ->
           ...
           _print_centre_distances ~label child p)
        all_octants
    | Leaf _ -> ()
end
